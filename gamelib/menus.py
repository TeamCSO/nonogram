...
def dibujar_menu(screen):
    ...
    cuadrado = pygame.image.load(os.path.join("images", "relleno.png")).convert_alpha()
    cruz = pygame.image.load(os.path.join("images", "equis.png")).convert_alpha()
    
    screen.blit(cuadrado, (50, 320))
    screen.blit(cruz, (50, 350))
    
    screen.blit(fuente2.render("- Credits:", True, (0, 0, 0)), (10, 400))
    screen.blit(fuente2.render("Created by Ealdor (ealdorj@gmail.com)", True, (0, 0, 0)), (50, 440))
...
def dibujar_menu2(screen, size):
    ...
    archivo = open(os.path.join('size', size))
    for line in archivo:
        for e in line:
            if e != ":" and not aux:
                titulo = titulo + e
